class ExternalTransactionsController < ApplicationController
  def index
    ...
    b.empty? ? @external_transactions = current_user.user_transactions : @external_transactions = current_user.user_transactions.where.not('id IN (?)', b).order(created_at: :desc)
  end
end
